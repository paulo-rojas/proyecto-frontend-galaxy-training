...
export class EscuelaEditarComponent implements OnInit {
  ...
  private escuelaConductorRequestDto!: EscuelaConductorRequestDto;
  ...
  private obtenerIdYCargarEscuela(): void {
    ...
    if (id) {
      this.escuelaId = +id; // Convertir a número
  ...
  private cargarEscuela(id: number): void {
    ...
    this.escuelaService.getEscuelaById(id).subscribe({
      next: (escuela: EscuelaConductorResponseDto) => {
  ...
  private cargarDatosEnFormulario(escuela: EscuelaConductorResponseDto): void {
    console.log('Cargando datos de la escuela en el formulario:', escuela);
  ...
  private cargarUbicacionPorNombres(escuela: EscuelaConductorResponseDto): void {
    ...
    if (departamento) {
      ...
      // Cargar provincias y buscar la provincia
      this.direccionService.getProvincias(departamento.id).subscribe({
        next: (provincias) => {
          ...
          if (provincia) {
            ...
            // Cargar distritos y buscar el distrito
  ...
  actualizarEscuela():void{
    console.log('Intentando actualizar la escuela...', this.registroForm.value);
